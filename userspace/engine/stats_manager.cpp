...
void stats_manager::print_stats(rule_collection* rules, std::string& out)
{
...
void stats_manager::on_event(rule_collection* rules, uint32_t rule_id)
{
    auto *rule = rules->get(rule_id);
    if (!rule)
    {
        throw falco_exception("on_event(): event with invalid rule_id: " + rule_id);
    }
